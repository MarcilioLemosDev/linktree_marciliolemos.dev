 ...
 * avermelhadas), brilho difuso somado (composite 'lighter') com espículas de
 * difração nas mais fortes, e paralaxe por profundidade no scroll.
 *
...
export function initEstrelas(canvas: HTMLCanvasElement): void {
  ...
  if (!movimento) {
    window.addEventListener('scroll', () => desenha(0), { passive: true });
  }
